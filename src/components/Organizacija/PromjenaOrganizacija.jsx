...
function PromjenaOrganizacija() {
    ...
    useEffect(() => {
        axios
          .get(`http://localhost:3001/organizacije/${id}`)
    ...
    const handleFormSubmit = async (event) => {
      event.preventDefault();
        try {
          await axios.patch(`http://localhost:3001/organizacije/${id}`, {
            ...formaPodaci,
          });
        } catch (error) {
    ...
    <form className={stil.forma} onSubmit={handleFormSubmit}>  
          <div className={stil.formaPrva} >    
            ...
            <div className={stil.successMessage}>
             Organizacija je uspješno uređena!
            </div>
            )}
          </div>
      </form>
...
export default PromjenaOrganizacija;